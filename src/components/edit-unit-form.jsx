...
export default function EditUnitForm({ unitId }) {
    ...
    try {
      ...
      if (data.success) {
        setFormData({
          ...
          baseUnit: data.unit.baseUnit || '',
          conversionFactor: data.unit.conversionFactor || 1,
          isBaseUnit: data.unit.isBaseUnit || false,
          isActive: data.unit.isActive !== undefined ? data.unit.isActive : true,
          sortOrder: data.unit.sortOrder || 100
        })
      } else {
      ...
      toast.error('Error loading unit data')
    } finally {
  ...
  const fetchBaseUnits = async () => {
    try {
      ...
      if (data.success) {
        setBaseUnits(data.units.filter(unit => unit.isBaseUnit && unit.id !== unitId))
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.push(`/dashboard/products/units/${unitId}`)
      } else {
  ...
  const availableBaseUnits = baseUnits.filter(unit => 
    unit.type === formData.type && unit.id !== unitId
  )
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Loading unit data...</p>
        </div>
      </div>
    )
  }
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              ...
              <Button 
                ...
                variant="outline" 
                className="w-full"
                onClick={() => router.push(`/dashboard/products/units/${unitId}`)}
                disabled={loading}
              >
